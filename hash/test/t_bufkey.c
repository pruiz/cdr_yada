...
/** \file t_bufkey.c
 *  test bufkey hash routines
 *
 * $Id: t_bufkey.c 14 2006-03-14 15:50:05Z grizz $
 */
...
int main(int argc, char **argv)
{
  ...
  if(!(hash = hash_create(0, bufkey, free)))
  ...
  for(loop=0; loop<ctr; loop++)
    ...
    if(hash_set(hash, string[loop], string[loop], strlen(string[loop])))
      errkill(-1, "failed");
    passed();
    }
  ...
  for(loop=0; loop<ctr; loop++)
    ...
    if(!(ptr = hash_get(hash, string[loop], strlen(string[loop]))))
      errkill(-1, "failed");
  ...
  testinfo("\ntesting hash reset");
  hash_reset(hash);
  if(hash_get(hash, chkstr, strlen(chkstr)))
