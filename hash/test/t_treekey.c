...
/** \file t_treekey.c
 *  test treekey hash routines
 *
 * $Id: t_treekey.c 11 2006-01-23 04:38:30Z shotgun $
 */
...
int main(int argc, char **argv)
{
  ...
  if(!(hash = hash_create(0, treekey, free)))
  ...
  for(loop=0; loop<ctr; loop++)
    {
    testinfo("storing [%u:%s]...", strlen(string[loop]), string[loop]);
    if(hash_set(hash, string[loop], strlen(string[loop]), string[loop]))
      errkill(-1, "failed");
    passed();
    }

  /* store NULL string */
  testinfo("storing NULL value...");
  if(hash_set(hash, strdup("testing"), 0, NULL))
    errkill(-1, "failed");
  passed();
  ...
  for(loop=0; loop<ctr; loop++)
    ...
    if(!(ptr = hash_get(hash, strlen(string[loop]), string[loop])))
      errkill(-1, "failed");
  ...
  /* test NULL string */
  testinfo("testing NULL string");
  if(!(ptr = hash_get(hash, 0, NULL)))
    errkill(-1, "failed");
  if(strcmp(ptr, "testing"))
    errkill(-1, "error");
  passed();

  testinfo("\ntesting hash reset");
  hash_reset(hash);
  if(hash_get(hash, strlen(chkstr), chkstr))
